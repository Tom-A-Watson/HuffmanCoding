...
public class Branch extends Node {
    ...
    public Map<Character, List<Boolean>> traverse(List<Boolean> list) {
        ...
        if (this.left != null) {                                        //if the left child exists
            ArrayList<Boolean> leftList = new ArrayList<>(list);            //create an Arraylist of booleans: 'leftList'
            leftList.add(false);                                        //add false to the list for each left traversal
            characterListMap.putAll(this.left.traverse(leftList));      //put all recorded left traversals (falses)
        }                                                               //into the map

        if (this.right != null) {                                       //if the right child exists
            ArrayList<Boolean> rightList = new ArrayList<>(list);       //create an ArrayList of booleans: 'rightList'
            rightList.add(true);                                        //add true to the list for each right traversal
            characterListMap.putAll(this.right.traverse(rightList));    //put all recorded right traversals (trues)
        }                                                               //into the map
